...
echo "=== カット4: gp_17 鮮魚（横長を活かしたレイアウト）(12.5-17.0s / メイン中盤) ==="
FRAMES4=$(python3 -c "print(int(${D4}*${FPS}))")
# 横長画像: ぼかし背景 + 中央にフル表示 + ゆっくりズームイン
ffmpeg -y -loop 1 -i gp_17.JPG -loop 1 -i gp_17.JPG \
  -filter_complex "\
  ...
  zoompan=z='1.0+0.1*on/${FRAMES4}':x='iw/2-(iw/zoom/2)':y='ih/2-(ih/zoom/2)':d=${FRAMES4}:s=${W}x${H}:fps=${FPS},\
  drawtext=fontfile=${FONT_BOLD}:text='今日のおすすめ、揃ってます':fontsize=48:fontcolor=white:borderw=3:bordercolor=black@0.6:x=(w-text_w)/2:y=h*0.82:alpha='if(lt(t,0.5),t/0.5,if(gt(t,${D4}-0.5),(${D4}-t)/0.5,1))'" \
